...
- [ ] Get the console output (or exact new symptom) from `WanderersVillage.console.exe` on Windows — the one remaining way to diagnose the actual Enter-crash root cause without Windows/Wine access in this environment. Once diagnosed and fixed, revert the console-wrapper export setting per the note above.
