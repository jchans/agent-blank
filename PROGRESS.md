...
- [ ] Get the user's confirmation that the Windows-export map-file fix above actually resolves their "crash after Enter" report — if not, get exact symptoms (does a window open at all? error dialog text? does it close immediately or hang?).
