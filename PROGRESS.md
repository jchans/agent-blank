...
- [x] Flags-only save/load (`user://save.json`, autosaved on every flag change). **Not the full "Save/load game state" item below** — player position, inventory, and other future state aren't persisted yet.
...
- [ ] JRPG battle system (turn-based) — separate scene, triggered from an encounter zone or dialogue action. This is the biggest remaining chunk of work; break it into its own sub-checklist here once started. Suggested first slice: a Battle.tscn with a text-only (ASCII-consistent!) menu of Attack/Run against one enemy type, HP numbers, no animation — get the state machine and scene transition working before adding depth.
- [ ] Full save/load game state: extend `game_state.gd` beyond flags to also persist player position/current map and any battle/inventory state once those systems exist. Keep it additive to the existing `flags` dict in save.json, don't redesign the format.
