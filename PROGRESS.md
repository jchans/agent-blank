...
- [x] Battle reward on win + a second enemy type: `GameState.gold` (persisted, additive to save.json), each `data/enemies/*.json` can set `"reward_gold"` (slime: 3, new `goblin.json`: 16 HP / 5 attack / 8 gold — tougher, for variety). Battle status line now also shows gold. Guard's dialogue offers a choice of slime or goblin. Verified headlessly (goblin loads correctly, gold goes from 0 to 8 on a win) with the usual temporary-test-hook approach.
...
- [ ] Battle system depth remaining: a real "you lost" consequence instead of a free full heal (e.g. lose some gold, or just return to the village with no gold penalty but no free heal either), more than one player action (e.g. a weak/strong attack, an item, once inventory exists). (Gold reward on win and a second enemy type are now done — see Done list.)
...
- [ ] Pause menu polish: a Settings/keybinds screen, maybe a save/quit-to-title option once a title screen "Continue" makes sense.
